<template>
  ...
  >
    ...
    <FormDialog
      v-model="roomDialog"
      :title="editingRoom ? 'Edit Network Room' : 'Add Network Room'"
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const rooms = ref<NetworkRoom[]>([]);
    const loading = ref(false);
    const roomDialog = ref(false);
    const deleteConfirmDialog = ref(false);
    const editingRoom = ref<NetworkRoom | null>(null);
    const searchTerm = ref("");
    const selectedRooms = ref<NetworkRoom[]>([]);
    ...
    const columns = [
      ...
      name: "floorName",
      label: "Floor",
      field: (row: NetworkRoom) => {
        const matchingFloor = floorOptions.value.find(
          (floor) => floor.id === row.floorId
        );
        return matchingFloor?.name || "N/A";
      },
      sortable: true,
    },
      {
    ...
    const fetchRooms = async () => {
      loading.value = true;
      try {
        rooms.value = await networkRoomService.getRoomsByCustomerId(
          props.customerId
        );
      } catch (error) {
        console.error("Failed to fetch network rooms:", error);
      } finally {
        loading.value = false;
      }
    };
    ...
    const openEditRoomDialog = (room: NetworkRoom) => {
    ...
    const saveRoom = async () => {
      try {
        if (
          !roomForm.value.name ||
          !roomForm.value.floorId ||
          roomForm.value.tilesWidth == null ||
          roomForm.value.tilesLength == null
        ) {
          throw new Error(
            "Please fill in all required fields: Name, Floor, Tiles Dimensions"
          );
        }
        ...
        if (editingRoom.value) {
          await networkRoomService.updateRoom(editingRoom.value.id, roomData);
        } else {
          await networkRoomService.createRoom(roomData);
        }
        ...
        console.error("Failed to save network room:", error);
    ...
    const confirmDeleteSingle = (id: number) => {
      ...
      deleteConfirmMessage.value =
        "Are you sure you want to delete this network room?";
      deleteConfirmDialog.value = true;
    };

    const confirmDeleteSelected = () => {
      deleteMode.value = "multiple";
      deleteConfirmMessage.value = `Are you sure you want to delete ${selectedRooms.value.length} network room(s)?`;
      deleteConfirmDialog.value = true;
    };
    ...
    const deleteSingleRoom = async (id: number) => {
      try {
        await networkRoomService.deleteRoom(id);
        await fetchRooms();
        emit("room-updated");
      } catch (error) {
        console.error("Failed to delete network room:", error);
      }
    };
    ...
    const deleteSelectedRooms = async () => {
      try {
        for (const room of selectedRooms.value) {
          await networkRoomService.deleteRoom(room.id);
        }
        await fetchRooms();
        selectedRooms.value = [];
        emit("room-updated");
      } catch (error) {
        console.error("Failed to delete selected network rooms:", error);
      }
    };
